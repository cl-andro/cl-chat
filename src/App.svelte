<script>
    import { state } from './lib/chat.svelte.js';
    import { deriveSeed, generateKeyPair } from './lib/crypto.js';
    import { connectServer } from './lib/network.js';
    import { login, addToast } from './lib/chat.svelte.js';
    import ChatScreen from './lib/ChatScreen.svelte';
    import AddContactModal from './lib/AddContactModal.svelte';
    ...
    async function handleAccess() {
        ...
        if (!window.crypto?.subtle) {
            addToast('HTTPS required for cryptography.', 'error');
            return;
        }
        if (typeof nacl === 'undefined') {
            addToast('Crypto library not loaded. Hard refresh.', 'error');
            return;
        }
